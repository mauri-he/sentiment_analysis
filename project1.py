...
def hinge_loss_single(feature_vector, label, theta, theta_0):
    ...
    Returns:
        ...
        parameters.
    """
    # Calculate the decision value for the data point
    decision_value = np.dot(theta, feature_vector) + theta_0
    
    # Calculate the hinge loss
    hinge_loss = max(0, 1 - label * decision_value)
...
def hinge_loss_full(feature_matrix, labels, theta, theta_0):
    ...
    total_loss = sum(hinge_loss_single(feature_matrix[i], labels[i], theta, theta_0) for i in range(len(labels)))
